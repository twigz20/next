...
describe('Testing /io/in/ endpoint', () => {
    ...
    describe('Testing POST', () => {
        ...
        test('Get file5.txt from io/to/dataurl route.', async () => {
            const req = {
                method: 'GET',
            };

            // This is a testing text.
            const url = `${domain}/io/to/dataurl/file5.txt?test=true`;
            let response = await makeRequestAllInternalError(url, req, page);

            expect(response.ok).toBeTruthy();
        });

        test('Archiving Root Directory from io/archive/ route.', async () => {
            const req = {
                method: 'GET',
            };

            // This is a testing text.
            const url = `${domain}/io/archive/?test=true`;
            let response = await makeRequestAllInternalError(url, req, page);

            expect(response.ok).toBeTruthy();
        });

        test('Reseting whole Filesystem from io/reset/ route.', async () => {
            const req = {
                method: 'GET',
            };

            // This is a testing text.
            const url = `${domain}/io/reset/?test=true`;
            let response = await makeRequestAllInternalError(url, req, page);
